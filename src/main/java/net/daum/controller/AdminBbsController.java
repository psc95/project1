...
public class AdminBbsController {
	...
	public ModelAndView admin_bbs_cont(int no,int page,String state,
		...
		if(isAdminLogin(session, response)) {
			...
			/* 문제) admin_bbs.xml에서 설정할 유일 아이디명이 abbs_cont이다. 번호를 기준으로 관리자 
			 * 자료실 DB레코드값을 가져오는 것을 만들어 보자.
			 */
			
			String bbs_cont = bc.getBbs_cont().replace("\n","<br>");/* textarea에서 엔터
			키를 친 부분을 줄바꿈 <br> 처리해서 웹브라우저로 보여진다. */
	...
	public ModelAndView admin_bbs_edit_ok(HttpServletRequest request,
			HttpServletResponse response, HttpSession session, BbsVO b)
	throws Exception{
		response.setContentType("text/html;charset=UTF-8");
